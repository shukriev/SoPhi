# ADR-019: `invoke_claude_code` tool for per-task implementation
...
An automated implementation pipeline needs to hand a well-scoped task off to an autonomous
coding session, so SoPhi can drive per-task implementation work in a target project without a
human present for each step. The original design considered a new dedicated module with
bespoke workspace provisioning and a hand-rolled orchestration loop — superseded by reaching
this entirely with existing pieces (`bash`, a Skill, and `sophi-schedule`'s Goal-mode task,
which runs through `PlanRunner` per ADR-018) plus exactly one new Tool.
...
2. **`riskLevel`/`ruleVerdict` are both hardcoded `DESTRUCTIVE`/`HIGH_RISK`**, never
   ...
   creating the scheduled task that uses it (ADR-016's "ask once, not per call").
...
4. **No orchestration code.** The per-task decomposition ("for each item, provision a
...
- No new orchestration surface to maintain — `sophi-schedule`'s existing Goal-mode task
  mechanism is the entire "loop over tasks" implementation.
- The nested Claude Code session's own actions are still gated by its own `--permission-mode
  auto`, not fully unattended — a task needing an action that mode won't auto-approve will
...
- Not yet empirically verified: whether `claude -p --permission-mode auto` fails safe to
  *deny* an action its classifier can't confidently decide, given there is no TTY to prompt
  ...
  regardless, but this should be tested directly once a real unattended task runs.
...
- ADR-018 (plan-and-execute) — `PlanRunner`, which does the actual per-task decomposition
