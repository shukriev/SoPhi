...
│  RuntimeBuilder: builtinTools/subagentDelegation/            │
│  goalDecomposition/skillTools/schedule (ADR-028)             │
└──────────────────────┬───────────────────────────────────────┘
                       │ AgentSession API
┌──────────────────────▼───────────────────────────────────────┐
...
| `sophi-skills` | Lazy-loaded Markdown skill packages; `SkillRegistry` merges global + project directories. `SkillVersionStore` (skill edit history/revert) is backed by `sophi-versioning`'s `VersionStore` — this module's first-ever Sophi-internal dependency (ADR-029). `SkillInstaller.install()` takes an additive `validate` callback that can reject a discovered skill's content before it's copied in (ADR-030) | complete |
...
| `sophi-versioning` | Versioned artifact history for skills, lessons, `HarnessConfig`, agent definitions, and memory-consolidation runs: `Version`/`VersionStore` (generic content + explicit `parentVersionId` + `producedBy`, append-only, revert-as-new-version) and a separate `Scorecard`/`ScorecardStore` for eval-suite measurements. Own leaf module depending only on `sophi-store`; every operation opens its ArcadeDB instance, does its work, and closes it immediately rather than holding it open for a process's lifetime, since CLI/companion/scheduler-daemon are separate processes that could each want it open at once (ADR-029). `ArtifactType.MEMORY_CONSOLIDATION` mirrors each consolidation run for audit consistency only — `sophi versions revert` on this type does not perform real memory recovery (ADR-031) | complete |
| `sophi-memory` | Declarative memory (Jane's Theory): MemoryTechnique SPI, JanesPalace rooms/salience/decay/profile, per-turn recall via ContextContributor, true deletion — best-effort, never breaks a turn. Storage is ArcadeDB (embedded document+graph+vector database) via `sophi-store`'s `ArcadeStore` primitive layer (ADR-026), replacing the original JSONL + brute-force-cosine storage; embedded-only, single-process. `Consolidator` optionally records a `sophi-versioning` `Version` per consolidation run, and `ConsolidationHistoryStore`'s `ConsolidationRecord` carries a stable `id` for that purpose (ADR-031) | complete |
...
| `sophi-cli` | Terminal CLI, TUI, slash commands, RPC mode. `sophi skill verify <id>\|--all` runs a trial skill version through the eval suite and reports a human-confirmed promote/revert/manual-review recommendation (ADR-030). `sophi memory consolidations list [<id>]` / `... restore <id>` browse consolidation-run history and bulk-recover everything one run soft-deleted (ADR-031) | complete |
| `sophi-web` | Web UI, WebSocket, SSE, REST endpoints | complete |
| `sophi-sdk` | Embeddable library for Spring `@Service` beans. `RuntimeBuilder` is the single place a host assembles its tool set: `.builtinTools(root, braveApiKey)` (file/bash/fetch/search/date), `.subagentDelegation()`, `.goalDecomposition(plansDir)`, `.skillTools()` — plus the pre-existing `.schedule(dir)`, `.mcpConfig(path)`, `.memory(...)`. `sophi-cli` and `sophi-companion` both call the same methods (ADR-028). `.configVersion(id, versionStore)` applies a versioned `HarnessConfig` (system prompt/temperature/max tokens/critic on-off/top-k skills/tool-description overrides) at build time (ADR-029). `WriteSkillTool`/`InstallSkillTool` gate on static content checks before writing and implement `Tool.confirmationPreview` (a diff / source summary shown instead of raw JSON); `verifySkill` runs the eval suite before/after a trial skill version (ADR-030) | complete |
| `sophi-companion` | OS tray/menu-bar desktop app (Compose Multiplatform Desktop) embedding `sophi-sdk` in-process: Chat/Sessions/MCP/Goals/Settings tabs, per-session `StateFlow` with one coroutine per turn, in-process `ScheduleEngine` poll loop, cross-platform native notifications, `jpackage` bundles. Standalone Gradle project — **not** in the root Maven `<modules>`; consumes `dev.sophi:sophi-sdk` via `mavenLocal()` (ADR-022). Shares `sophi-cli`'s full tool surface (ADR-028): builtin file/bash/fetch tools sandboxed to a configurable `workspaceDir` setting (default `~/.sophi/workspace`), calendar, skill invocation, subagent delegation, and goal decomposition — resolved per-turn via `SessionIdContext` (moved to `sophi-core` for this) rather than one hardcoded session id, since companion runs many concurrent chat sessions. Voice mode (`dev.sophi.companion.voice`): push-to-talk turns via local `whisper.cpp`/`piper` subprocesses (`VoiceController`, sentence-by-sentence TTS playback as the reply streams), with a `VoiceInstaller` that downloads, checksum-verifies, and extracts the tool binaries + models from a pinned GitHub release manifest into `~/.sophi/voice` — zero manual setup for the common case, driven from the Settings tab. The schedule-poll loop now surfaces a notification whenever background memory consolidation actually changes something, instead of running silently (ADR-031). `CompanionSettings` holds a `profiles: List<LlmProfile>` (each a full LLM + memory connection) plus `activeProfileName`, replacing the earlier duplicated top-level LLM/memory fields; `SettingsStore.load()` migrates any pre-profiles file into a single synthesized `"Default"` profile, and the Settings tab exposes one unified profile editor (dropdown selector, Save/Set Active/+New/Delete, dirty-edit discard confirmation) instead of separate active-connection/saved-profiles/memory sections. A companion-only, macOS-only check-in scheduler (`CheckInScheduler`, cron-driven via `dev.sophi.schedule.model.CronSchedules`) pops a native `osascript` text-entry dialog at configured times and records the reply into an Obsidian vault via a one-off, vault-scoped `SophiRuntime` turn (`obsidianVaultPath`/`jiraBaseUrl`/`checkIns` settings — config-file only, no Settings-tab UI yet). Ambient listening (`ambientListeningEnabled`, ADR-034): a new `AmbientListener` records fixed-duration mic clips through the same `AudioRecorder`/`WhisperTranscriber` while the window is open, filters whisper's known silence-hallucination artifacts, and writes notable batches to memory via `SophiRuntime.settleExternalTurn` tagged with a new `ambient` flag so `SignificanceEncoder` can classify third-party content correctly instead of defaulting to `USER_DIRECT`; reminder-shaped batches additionally run one turn on a second, memory-disabled `SophiRuntime` whose only tool is the existing `manage_scheduled_task` | complete |
...
data class HookContext(
    ...
    val ambient: Boolean = false         // AFTER_TURN, ADR-034: overheard speech, not a real reply
)
...
`TurnObservation` and `BrowseFilter` both gained one field for ambient listening (ADR-034):
`TurnObservation.ambient` (default `false`, threaded from `HookContext` by `MemoryPlugin`'s
`AFTER_TURN` hook) tells `SignificanceEncoder` the exchange is overheard speech rather than a real
chat turn, so it can classify third-party content as `THIRD_PARTY`/`ENTITIES` instead of defaulting
to `USER_DIRECT`; `BrowseFilter.provenance` lets a caller (the companion's Memory tab) browse by
that same field.
...
| [ADR-030](adr/ADR-030-skill-write-admission-gate.md) | Skill-write admission gate (Phase 2) | Static content checks (secret scan, size cap, prompt-injection scan for third-party content) block a write before it lands; `InstallSkillTool` gains the versioning `WriteSkillTool` already had; `sophi skill verify` runs the eval suite before/after a trial skill version with its own acceptance rule (no improvement requirement, unlike `Tournament.evaluateAcceptance`) plus zero-coverage detection; every promote/revert requires human confirmation; `Tool.confirmationPreview` replaces raw-JSON confirmation prompts where a tool provides one — corrected two of the roadmap's own stale premises (skills were already reachable from every host; writes were already human-confirmed, just not reviewable) |
| [ADR-031](adr/ADR-031-memory-consolidation-visibility.md) | Memory consolidation visibility, bulk recovery & audit consistency (Phase 3) | Corrected the roadmap's premise that `Consolidator.compress` is an unauthorized orchestrator mutation needing a confirmation gate — the orchestrator never calls `consolidate()` at all, and most of the "gate" (soft-delete/grace/purge, `ForgetEngine.restore`, `autoPurgeEnabled`, a full audit log) already shipped; the companion's previously-silent poll-loop consolidation now notifies; `sophi memory consolidations list/restore` browse history and bulk-recover a run via the existing `restore(id)` path; `ArtifactType.MEMORY_CONSOLIDATION` mirrors each run into `sophi-versioning` for audit consistency only — `sophi versions revert` explicitly does not perform real recovery for this type |
| [ADR-033](adr/ADR-033-personalized-proactive-rescue.md) | Personalized proactive rescue | `Consolidator` gains a `classifyPatterns` step (repetition-gated by `SalienceSignals.rep`, not per-turn encode time — a single utterance isn't a pattern); `Memory.actionablePattern` flag with an unconditional PERSONAL-or-below sensitivity ceiling on its read surface (`JanesPalace.actionablePatterns()`) since it reaches an OS notification, a more exposed surface than chat; one new `list_actionable_patterns` tool; ships as an ordinary user-created `TaskMode.Goal` scheduled task, no new task mode; one-line guard on `ScheduleEngine`'s existing notifier call suppresses the `NO_RESCUE_NEEDED` sentinel so a daily check doesn't ping for nothing |
| [ADR-034](adr/ADR-034-ambient-listening.md) | Ambient listening — passive memory and reminders from overheard speech | A skill-file design was rejected — `SignificanceEncoder` is a separate hardcoded-prompt call the agent never sees, and defaults every memory to `USER_DIRECT`; instead an `ambient` flag threads through `HookContext`/`TurnObservation` into a new encoder prompt variant; VAD rejected in favor of fixed-duration clips through the existing `AudioRecorder`/`WhisperTranscriber`; memory writes go through the existing memory-enabled `SophiRuntime.settleExternalTurn` only, reminder-shaped batches run on a second, memory-disabled `SophiRuntime` (avoids ArcadeDB's single-open lock and double-encoding) whose only tool is the existing `manage_scheduled_task`; fixed a pre-existing `VoiceController` bug (unhandled mic-busy exception could wedge push-to-talk forever) that this feature's mic contention makes reachable |
| [ADR-035](adr/ADR-035-commitment-tracking.md) | Commitment tracking (chat-only v1) | `SignificanceEncoder` gains a `commitment` judgment classified at write time, not consolidation-time (a single utterance is already complete evidence, unlike actionable patterns); `Memory.isCommitment` flag gated in code to `Provenance.USER_DIRECT` regardless of encoder output — ambient stays out until diarization/consent exist; `JanesPalace.openCommitments()` reuses `actionablePatterns()`'s unconditional PERSONAL-or-below sensitivity ceiling plus a `commitmentExpiryMs` age cutoff so nudges don't run forever; one new `list_open_commitments` tool; ships as an ordinary user-created `TaskMode.Goal` scheduled task using the pre-existing `manage_scheduled_task` tool and `GoalsTab` — no new store, timer, or companion tab |
...
| Skill-write admission gate — static content checks, `sophi skill verify`, confirmation previews — Phase 2 (ADR-030) | post-M7 | complete | [article-31](articles/article-31.md) |
| Memory consolidation visibility, bulk recovery & audit consistency — Phase 3 (ADR-031) | post-M7 | complete | [article-32](articles/article-32.md) |
| `sophi-companion` — periodic native-dialog check-ins logged to an Obsidian vault | post-M7 | complete | — |
| `sophi-companion` — unified LLM + memory profile settings (`LlmProfile`, replaces duplicated top-level fields) | post-M7 | complete | — |
| `sophi-companion`/`sophi-memory` — ambient listening, passive third-party-attributed memory + reminders (ADR-034) | post-M7 | complete (manual verification pending) | [article-34](articles/article-34.md) |
