...
private fun versions(home: Path, project: Boolean) = SkillVersionStore(
    VersionStore(home.resolve(".versions")), project, legacyJsonlPath = home.resolve(".versions.jsonl")
)
...
class SkillReviewCommandTest : FunSpec({
    test("review prints invocation and adjacent-failure counts for a skill with a version") {
        ...
        val version = versions(skillsHome, project = false).record(SkillVersion(skillId = "site-a", project = false, content = "c"))
        SkillInvocationStore(skillsHome.resolve(".invocations.jsonl"))
            .record(SkillInvocationEvent(ts = version.ts + 100, sessionId = "s1", skillId = "site-a"))
        writeToolEvent(learningHome, ToolEvent(ts = version.ts + 200, scope = "/p", sessionId = "s1", tool = "write_file", success = false, durationMillis = 1))
    ...
    test("review persists both snapshot files, fully overwritten not appended, across two runs") {
        ...
        versions(skillsHome, project = false).record(SkillVersion(skillId = "site-a", project = false, content = "c"))
        SkillInvocationStore(skillsHome.resolve(".invocations.jsonl"))
    ...
    test("a project-scoped version is attributed even when a same-id global version also exists") {
        ...
        val globalVersion = versions(skillsHome, project = false)
            .record(SkillVersion(skillId = "site-a", project = false, content = "global"))
        val projectVersion = versions(projectSkillsHome, project = true)
            .record(SkillVersion(skillId = "site-a", project = true, content = "project"))
        SkillInvocationStore(skillsHome.resolve(".invocations.jsonl"))
            .record(SkillInvocationEvent(ts = maxOf(globalVersion.ts, projectVersion.ts) + 100, sessionId = "s1", skillId = "site-a"))

        val lines = mutableListOf<String>()
        SkillReview(skillsHome, projectSkillsHome, learningHome, filterId = null) { lines.add(it) }.run()

        lines.any { it.contains(projectVersion.id) && it.contains("invocations=1") } shouldBe true
        lines.none { it.contains(globalVersion.id) } shouldBe true
    }
    ...
    test("passing a filterId narrows printed output without changing either persisted snapshot") {
        ...
        versions(skillsHome, project = false).record(SkillVersion(skillId = "site-a", project = false, content = "c"))
        versions(skillsHome, project = false).record(SkillVersion(skillId = "site-b", project = false, content = "c"))
    ...
    test("the persisted attribution snapshot round-trips back into SkillVersionAttribution") {
        ...
        versions(skillsHome, project = false)
            .record(SkillVersion(skillId = "site-a", project = false, content = "c", trial = true))
