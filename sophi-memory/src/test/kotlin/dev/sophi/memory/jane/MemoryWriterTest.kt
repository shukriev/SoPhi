...
class MemoryWriterTest : FunSpec({
    ...
    fun rig(): Triple<PalaceStore, EmbeddingIndex, MemoryWriter> {
        val store = PalaceStore(tempdir().toPath())
        val index = EmbeddingIndex()
        val writer = MemoryWriter(store, index, UserProfile(store), embeddings, "fake", JanesPalaceConfig())
        return Triple(store, index, writer)
    }
    ...
    test("high-signal memory is stored with blended salience; embedding and index updated") {
        val (store, index, writer) = rig()
        ...
        index.get(m.id) shouldBe store.embeddings().getValue(m.id)
    }
