...
configurations.all {
    resolutionStrategy {
        // arcadedb-engine's SQL grammar classes are pre-generated by the ANTLR *tool* at 4.9.1
        // and embed a serialized parser table (ATN) in that version's binary format. Gradle's
        // default "highest wins" conflict resolution otherwise picks 4.13.1 — pulled in
        // transitively by spring-ai-model's (unused here) StringTemplate support — whose
        // ATNDeserializer dropped the ability to read that older format, so every ArcadeDB SQL
        // query throws `NoClassDefFoundError: Could not initialize class ...SQLLexer` at runtime.
        // Force the version arcadedb-engine actually needs; nothing in this codebase uses Spring
        // AI's StringTemplate-based PromptTemplate, so downgrading it here is safe.
        force("org.antlr:antlr4-runtime:4.9.1")
    }
}
